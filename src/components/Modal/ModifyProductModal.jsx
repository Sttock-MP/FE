...
const wrapperStyle = 'flex justify-between'
...
function ModifyProductModal({ onClose, item }) {
  ...
  const [productName, setProductName] = useState(item?.description)
  const [category, setCategory] = useState(
    CATEGORY_LIST.filter((el) => el.value === item?.category)[0]
  )
  const [product, setProduct] = useState(
    SUB_CATEGORY_LIST.filter((el) => el.category.value === category?.value)[0]
  )
  const [startDate, setStartDate] = useState(new Date(item?.purchaseDate))
  ...
  return (
    <>
      ...
      <div className="fixed top-[15%] left-[17.5%] w-[900px] h-[560px] border border-brown bg-white  justify-between flex p-[4rem] flex-col gap-1">
        <p className="text-2xl font-bold text-left text-darkBrown">상품추가하기</p>
        <div className="h-[100%] flex flex-col px-[4rem] gap-[1rem] my-[3rem]">
          ...
          <div className={wrapperStyle}>
            <p className="text-sm font-medium text-left text-darkBrown">상품별칭</p>
            <input
          ...
          <div className={wrapperStyle}>
            <p className="text-sm font-medium text-left text-darkBrown">계산기준</p>
            <div className="w-[200px] border border-borderColor h-[28px] text-sm font-medium">
              {product?.standard?.value}
            </div>
          </div>
          <div className={wrapperStyle}>
